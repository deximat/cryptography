...
public class SolovayStrassenPrimalityTest {
	...
	private boolean isPrime(BigInteger numberToTest) {
		...
		// this will always be integer because it is odd number - 1 and then
		// divided by 2
		...
		for (int i = 0; i < numberOfIterations; i++) {
			...
			randomNumber = randomNumber.mod(numberToTestMinusOne).add(
					BigInteger.ONE);
			// calculate jacobi
	...
	private BigInteger modPowerDummy(BigInteger base, BigInteger exponent,
			BigInteger mod) {
		BigInteger solution = BigInteger.ONE;
		for (BigInteger counter = BigInteger.ZERO; counter.compareTo(exponent) != 0; counter = counter
				.add(BigInteger.ONE)) {
	...
	private BigInteger calculateJacobi(BigInteger a, BigInteger b) {
		BigInteger solution = BigInteger.ONE;
		...
		while (!a.equals(BigInteger.ZERO)) {
			...
			while (a.mod(BigInteger.valueOf(2)).equals(BigInteger.ZERO)) {
				...
				if (bMod8.equals(BigInteger.valueOf(3))
						|| bMod8.equals(BigInteger.valueOf(5))) {
			...
			// if we swapped a and b we should invert sign of solution if next
			// rule is satisfied
			if (a.mod(BigInteger.valueOf(4)).equals(BigInteger.valueOf(3))
					&& b.mod(BigInteger.valueOf(4)).equals(
	...
	public static void main(String[] args) {
		SolovayStrassenPrimalityTest primalityTest = new SolovayStrassenPrimalityTest();
		for (int i = 0; i < 100; i++) {
